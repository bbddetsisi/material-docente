...
para localizar físicamente el dispositivo dentro de la parcela, se anota la fila
en la que está localizado el dispositivo, la altura a la que está instalado (en
centímetros) y, opcionalmente, la columna en la que se encuentra (en caso de que
el dispositivo se aplique a las viñas de una localización más específica que
únicamente una fila). Es importante poder ordenar dichos dispositivos a la hora
de mostrarlos por pantalla, y que dicho orden sea configurable por cada usuario,
aunque por defecto será por número de serie de forma ascendente. Por seguridad,
el sistema no debe permitir borrar una parcela si esta tiene dispositivos
asociados.
...
y la unidad en la que se expresa (ej. grados centígrados, milímetros,
porcentaje).
...
de techo abierto (OTC). Un dispositivo puede estar opcionalmente asociado a un
único tratamiento de viña que está midiendo en ese momento.

Los sensores envían información constantemente, cada uno a una frecuencia
potencialmente diferente (desde 0,01 Hz hasta 0,5 Hz), generando lecturas. De
cada lectura nos interesa guardar el sensor que la produjo, la fecha y hora
exacta (timestamp) en la que se tomó, y el valor numérico registrado. La
combinación de un sensor concreto y una fecha/hora exacta determina una lectura
de forma única. Si se elimina un sensor, se debe eliminar todo su histórico de
lecturas.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{figs/modelado/ejercicio-18}
\end{figure}
